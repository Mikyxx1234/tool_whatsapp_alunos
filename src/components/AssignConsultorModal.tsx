...
export function AssignConsultorModal({ open, item, role, categoria, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    // Combina nomes ja gravados no banco (qualquer pessoa) + lista de
    // consultores academicos vinda do dcz (so chega quando role=admin).
    const fromDcz = getConsultoresAcademicos();
    fetchConsultoresDistintos()
      .then((r) => {
        if (cancelled) return;
        const merged = Array.from(new Set([...(r.consultores || []), ...fromDcz]))
          .filter(Boolean)
      ...
      .catch(() => {
        if (!cancelled) {
          setSugestoes(fromDcz.slice().sort((a, b) => a.localeCompare(b, 'pt-BR', { sensitivity: 'base' })));
        }
      });
    return () => {
      cancelled = true;
